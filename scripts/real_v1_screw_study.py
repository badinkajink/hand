...
A screw sits in a countersink, and the seat centres, wedges and constrains. So:
  ...
  regrip    0..2 mm of re-squeeze after the turn         how much grip does the insertion need?
...
BASE = dict(obj="screwdriver_medium", lift=0.10, angle_deg=-90.0, axis_k=0.25, turn_steps=550,
            budget=0.5, hold_steps=500, gap=0.002, carry_squeeze=0.0003, press_mm=10.0,
...
def main() -> int:
    ...
    ap.add_argument("--arms", default="press,regrip,capture,seat,distance,angle,endurance")
    args = ap.parse_args()
    ...
    if "regrip" in arms:
        for cs in (0.0, 0.0003, 0.0005, 0.001, 0.002):
            add("regrip", args.reps, carry_squeeze=cs, **SEAT)
    if "capture" in arms:
    ...
    for arm in sorted({r["arm"] for r in rows}):
        ...
        keys = sorted({(r.get("press_mm"), tuple(r.get("place_err_mm") or ()),
                        r.get("carry_squeeze_mm"), r.get("seat_z"),
                        tuple(r.get("place_xy") or ()),
                        r.get("tip_len_mm"), r.get("cycles_asked")) for r in sub},
                      key=str)
        for k in keys:
            g = [r for r in sub
                 if (r.get("press_mm"), tuple(r.get("place_err_mm") or ()),
                     r.get("carry_squeeze_mm"), r.get("seat_z"),
