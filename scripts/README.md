...
| `real_v1_basketball_grasp.py` | a friction CAP grasp of a basketball (sizes 3-7) on a real_v1 gantry layout: `fit` builds a forward-kinematic reach map per finger and picks three contacts symmetric about the thumb at a wrap angle, `sweep` closes / lifts / holds over friction, wrap angle, ball size, layout and BOTH plants (shipped kp 30 and measured kp 0.5 / 0.35 N m), `film` renders one case. Runs the MuJoCo grasp contact setting (elliptic, impratio 10) because at the repo default the friction constraint drains the pads before the ball leaves the floor |
| `real_v1_basketball_media.py` / `real_v1_basketball_page.py` | films, pose snapshots, the contact-model sensitivity, and the page `docs/experiments/20260916-basketball/20260916-basketball_grasp.html` from the sweep's own JSON |
...
| `real_v1_tag_tracker.py` | 6. **the object-pose sensor.** Two AprilTags -> cos(shaft, up), absolute height and slip, at 0.017 deg / 0.030 mm rms, pushed live to the station. Geometry lives in `morphohand.bench.tags`. `--probe` first, with `--tag-end` so the `--shaft-axis` sign is CHECKED and not narrated; `--video-hz 4` also tapes IR frames for replay |
| `real_v1_tracker_service.py` | the workstation companion the CB1 calls: one camera, `/start` on a run id, `/stop`. A reorientation that cannot arm the camera is refused rather than run blind. `--post-roll` keeps recording past the end of the motion so `cos_hold` has a hold to measure |
| `real_v1_tracker_replay.py` | a taped run -> an annotated mp4 and a phase-aligned filmstrip. The trace says the shaft turned 55.9 deg; only the pictures say whether it pivoted about a contact or about the floor |
| `real_v1_transfer_study.py` | the pre-registered analysis of sim-vs-bench hand RANKING (protocol: `docs/experiments/20260831-real_v1-transfer-protocol/`). Written before any trial existed; runs on partial data |
| `paper_morphology_diversity.py` | how far apart the eight deployed hands are. `fig_population_plane` is the PAPER figure (the eight inside the population in thumb-to-pair span x pair opening, the two coordinates retention actually constrains), with `fig_hand_map` (MDS of the eight alone) and `fig_hand_cards` (full-width per-hand plate) as alternatives; the other four — mount PCA, palm-plane facets, a random-eight spread null, grasp-mechanics PCA — are kept behind `\iffalse` in `paper/morphology_diversity.tex`. Writes `paper/figures_diversity/*.pdf` + `numbers.txt`. Read-only over the sobol8192 manifest and the exported plans; bench numbers are parsed from `paper/transfer_table.tex` because the transfer-firstpass `data.json` is stale |
