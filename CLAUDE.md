...
## Result pages — local file first, artifact second

Every result page is a **file in this repo** (`docs/experiments/<YYYYMMDD-topic>/<YYYYMMDD-name>.html`,
regenerated by a builder script) and the claude.ai artifact is a copy of that file republished to
the same URL. **Always quote the local path next to the URL** — the user will not accept results
that live only on claude.ai. `docs/experiments/INDEX.md` is the register: one row per page, local
file and artifact URL, newest first; update it whenever a page is published or renamed.
Date-prefix the **filename**, not just the folder, so a flat listing sorts. Titles are
descriptive noun phrases in the style of `paper/hand_iros26-4.pdf`, not images or phrases.
