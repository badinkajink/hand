...
\title{A Reconfigurable Hand for Validation of Simulation-Optimized Morphologies}
...
Hardware (motors, rails, custom mounts, fingertip tooling): approximately \$800--\$1{,}500.
Compute (GPU time for parallel simulation and quality-diversity search): \$500--\$1000
in cloud credits or equivalent on-premise GPU time.

\smallskip
\textbf{H7. How long will it take?}\\
Twelve months hopefully: six-to-nine months for hardware design and fabrication,
six-to-nine concurrent months for the simulation and optimization framework, three months for thorough experiments and writing.
...
No two human hands are made the same.
The positions of finger bases on the palm, the lengths of each phalange segment, the range
of side-to-side spread at each knuckle---these quantities determine how well a hand can grasp something, or whether it can grasp it at all.
...
I choose for my task distribution?'' remains largely unanswered in the real, physical world.
...
\begin{enumerate}
  ...
  \item Is explicitly designed as a \emph{measurement instrument} for morphology
    evaluation, in addition to an operational end-effector.
\end{enumerate}
...
The primary design intent is for the platform to be a measurement instrument, then an operational robot hand.
This distinction drives some key design decisions.
...
Thus, the platform makes several compromises: the gantry mechanism adds bulk and weight; the telescoping phalange actuator adds
minor compliance not present in a rigid link (\textit{wx: while true we hope to minimize this}); the total DOF count (18) and accompanying mechanical complexity will make highly dynamic manipulation likely impractical.
...
finger is coupled to the PIP joint during natural flexion and extension, and omitting it reduces DOF count by 3 (one per finger) without meaningfully reducing the
control and morphology space we can explore.
...
\begin{table}[t]
  ...
  \begin{tabular}{lllll}
    ...
    \midrule
    Base X (lateral)  & $x_i$    & $\pm$60\,mm & Stepper+leadscrew & Morphology \\
    Base Y (fore-aft) & $y_i$    & $\pm$40\,mm & Stepper+leadscrew & Morphology \\
    Yaw (ABD/ADD)     & $\psi_i$ & $\pm$45$^\circ$ & Servo & Control \\
    MCP flex/ext      & $q^p_i$  & 0--110$^\circ$ & Servo & Control \\
    Prox.\ length     & $l_i$    & 40--90\,mm  & Linear act./stepper & Morphology \\
    PIP flex/ext      & $q^d_i$  & 0--100$^\circ$ & Servo & Control \\
    \midrule
...
The 18 DOFs are split into two categories:
\begin{itemize}
  \item \textbf{Morphological DOFs} (9 total: $x_i, y_i, l_i$ for $i \in \{T,I,M\}$):
    these define the hand's \emph{shape} and are the variables the outer morphology
    optimizer controls. Practically, they are not ``control" DoFs because they are lead-screw driven to achieve holding torque across a large motion range and are thus slow to change, and because they would not typically be available for on-the-fly change in a robotic hand.
  \item \textbf{Control DOFs} (9 total: $\psi_i, q^p_i, q^d_i$ for $i \in \{T,I,M\}$):
